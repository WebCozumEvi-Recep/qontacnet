...
interface Props {
  ...
  /** Baskı çıktısı: zemini basılmayacak yüzde zemin çizilmez */
  baski?: boolean;
}
...
export const KartTuvali = forwardRef<HTMLCanvasElement | null, Props>(function KartTuvali(
  { sablon, yuz, degerler, vurgu, className = "", onIsaretci, baski = false }, ref,
) {
  ...
  const qrAlani = yuzAlanlari.find(a => a.tip === "qr");
  ...
  useEffect(() => {
    ...
    Promise.all(kaynaklar.map(gorselYukle)).then(([img, ...digerleri]) => {
      ...
      yuzCiz(ctx, sablon, yuz, img, degerler, qr.current, harita, vurgu, baski);
    });
    return () => { iptal = true; };
  }, [sablon, yuz, degerler, vurgu, zemin, resimler, surum, baski]);
  ...
  return (
    <>
      <canvas
        ...
        width={w}
        height={h}
        className={`w-full h-auto rounded-xl shadow-lg ${baski ? "dama" : ""} ${onIsaretci ? "cursor-move touch-none" : ""} ${className}`}
        style={{ aspectRatio: `${w} / ${h}` }}
        onPointerDown={olay("bas")}
        onPointerMove={e => { if (e.buttons) olay("surukle")(e); }}
        onPointerUp={olay("birak")}
      />
      {/* QR, çizimde kaynak olarak kullanılır; sessiz bölgeli. Zemin boşsa şeffaf çizilir. */}
      <QRCodeCanvas ref={qr} value={degerler.qr || " "} size={600} marginSize={2} level="M"
        bgColor={qrAlani?.zemin || "transparent"} fgColor={qrAlani?.renk ?? "#000000"} style={{ display: "none" }} />
    </>
  );
});
