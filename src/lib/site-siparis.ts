...
export interface SiteSiparisSecenek {
  ...
  firmaId?: string | null;
  /** Firma sayfasında yalnız firmaya tanımlı ürün satılabilir. */
  izinliUrunId?: string;
  kaynak: "SITE" | "FIRMA_LINK";
  donusYolu: string;
}

// Site üzerinden ürün satın alma: doğrulama → sipariş (ödeme bekliyor) → 3D ödeme başlatma.
// Ana sayfa (/api/siparis) ve firma satış sayfası (/api/f/[id]/siparis) ortak kullanır.
export async function siteSiparisiOlustur(req: Request, body: Record<string, unknown>, sec: SiteSiparisSecenek) {
  try {
    ...
    if (sec.izinliUrunId && String(urunId) !== sec.izinliUrunId) {
      return NextResponse.json({ ok: false, error: "Bu ürün bu sayfadan satın alınamaz." }, { status: 400 });
    }
    ...
    const hesapToken = sec.kaynak === "FIRMA_LINK" ? randomBytes(24).toString("base64url") : null;
    ...
    const odeme = await odemeBaslat({
      ...
      donusYolu: sec.donusYolu,
    });

    return NextResponse.json({ ok: true, siparisNo: order.siparisNo, hesapToken, odeme });
  } catch (e) {
