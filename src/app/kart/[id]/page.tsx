...
export default function KartPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const color = member.kartRenk;

  const saveContact = () => {
    const vCard = `BEGIN:VCARD\nVERSION:3.0\nFN:${member.ad} ${member.soyad}\nTITLE:${member.unvan}\nORG:${member.firmaAdi}\nTEL:${member.telefon}\nEMAIL:${member.email}\nURL:https://qontac.net/kart/${member.id}\nEND:VCARD`;
    const blob = new Blob([vCard], { type: "text/vcard" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `${member.ad}_${member.soyad}.vcf`;
    a.click();
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/lead", {
        ...
        body: JSON.stringify({ uyeId: member.id, ...leadForm }),
      });
  ...
  const links = [
    member.whatsapp && { icon: "chat", label: "WhatsApp ile Yaz", href: `https://wa.me/${member.whatsapp.replace(/\s/g, "")}`, color: "#25d366" },
    member.linkedin && { icon: "link", label: "LinkedIn", href: `https://${member.linkedin}`, color: "#0077b5" },
    member.instagram && { icon: "photo_camera", label: "Instagram", href: `https://instagram.com/${member.instagram.replace("@", "")}`, color: "#e1306c" },
    member.website && { icon: "public", label: "Website", href: `https://${member.website}`, color: "#00d4ff" },
  ].filter(Boolean) as { icon: string; label: string; href: string; color: string }[];

  return (
    <div className="min-h-screen bg-background flex flex-col items-center" style={{ background: "#050816" }}>
      {/* Top glow */}
      ...
      <div className="w-full max-w-sm mx-auto px-4 py-8 relative z-10">
        ...
        <div className="glass-card rounded-[2rem] p-6 mb-4 text-center relative overflow-hidden">
          ...
          <h1 className="text-2xl font-bold text-on-surface mb-1" style={{ fontFamily: "Sora, sans-serif" }}>
            {member.ad} {member.soyad}
          </h1>
          <p className="text-sm font-medium mb-1" style={{ color }}>{member.unvan}</p>
          <p className="text-sm text-on-surface-variant">{member.firmaAdi}</p>

          {member.biyografi && (
            <p className="text-xs text-on-surface-variant mt-3 leading-relaxed border-t border-white/10 pt-3">
              {member.biyografi}
            </p>
          )}
        </div>
        ...
        {/* Contact buttons */}
        {member.telefon && (
          <a href={`tel:${member.telefon}`}
            className="w-full flex items-center gap-3 py-4 px-5 glass-card rounded-2xl mb-3 hover:bg-white/5 transition-all group">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: `${color}15` }}>
              <span className="material-symbols-outlined text-xl" style={{ color }}>call</span>
            </div>
            <div>
              <p className="text-xs text-on-surface-variant">Ara</p>
              <p className="text-sm text-on-surface font-medium">{member.telefon}</p>
            </div>
        ...
        {member.email && (
          <a href={`mailto:${member.email}`}
            className="w-full flex items-center gap-3 py-4 px-5 glass-card rounded-2xl mb-4 hover:bg-white/5 transition-all group">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: `${color}15` }}>
              <span className="material-symbols-outlined text-xl" style={{ color }}>mail</span>
            </div>
            <div>
              <p className="text-xs text-on-surface-variant">E-Posta</p>
              <p className="text-sm text-on-surface font-medium">{member.email}</p>
            </div>
            <span className="material-symbols-outlined text-on-surface-variant/40 ml-auto group-hover:text-primary transition-all">chevron_right</span>
          </a>
        )}
        ...
        <div className="text-center">
          <Link href="/" className="inline-flex items-center gap-2 text-xs text-on-surface-variant/50 hover:text-on-surface-variant transition-all">
            ...
            <span>·</span>
            <span>Dijital Kartvizit Oluştur</span>
          </Link>
        </div>
      </div>
          ...
          <div className="w-full max-w-sm glass-card rounded-t-3xl p-6 pb-8" onClick={e => e.stopPropagation()}>
            {leadSaved ? (
              <div className="text-center py-6">
                ...
                <p className="text-sm text-on-surface-variant mt-1">{member.ad} en kısa sürede sizinle iletişime geçecek.</p>
              </div>
            ) : (
              <>
                <div className="flex items-center justify-between mb-5">
                  <div>
                    ...
                    <p className="text-xs text-on-surface-variant">{member.ad} ile iletişim bilgilerinizi paylaşın</p>
                  </div>
                ...
                <form onSubmit={handleLeadSubmit} className="space-y-3">
                  ...
                  <button type="submit" disabled={leadLoading}
                    className="w-full py-3.5 font-bold rounded-xl text-sm hover:scale-[1.02] active:scale-[0.98] transition-all mt-2 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                    style={{ background: color, color: "#000" }}>
                    {leadLoading ? (
                      <>
