"use client";
import { mockCardBatches, BatchDurum } from "@/lib/mock-data";

const durumMap: Record<BatchDurum, { label: string; color: string }> = {
...
export default function AdminKartlarPage() {
  const toplam = mockCardBatches.reduce((a, b) => a + b.miktar, 0);
  const stokta = mockCardBatches.filter(b => b.durum === "stokta").reduce((a, b) => a + b.miktar, 0);
  const tahsis = mockCardBatches.filter(b => b.durum === "tahsis").reduce((a, b) => a + b.miktar, 0);
  const uretimde = mockCardBatches.filter(b => b.durum === "uretimde").reduce((a, b) => a + b.miktar, 0);
  ...
  return (
    <div className="space-y-6 max-w-[1200px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Stat icon="warehouse" label="Stokta" value={stokta} color="#42faba" />
        <Stat icon="assignment_turned_in" label="Tahsis Edildi" value={tahsis} color="#6001d1" />
        <Stat icon="factory" label="Üretimde" value={uretimde} color="#a8e8ff" />
      </div>
        ...
        {mockCardBatches.map(b => (
          <div key={b.id} className="glass-card rounded-2xl p-5 hover:border-primary/20 transition-all">
            <div className="flex items-start justify-between mb-4">
              ...
              <span className="text-xs px-2 py-1 rounded-full whitespace-nowrap" style={{ background: `${durumMap[b.durum].color}15`, color: durumMap[b.durum].color, border: `1px solid ${durumMap[b.durum].color}30` }}>
                {durumMap[b.durum].label}
              </span>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                <p className="text-xs text-on-surface-variant">Miktar</p>
                <p className="text-on-surface font-medium">{b.miktar.toLocaleString("tr-TR")} adet</p>
              </div>
              <div>
                <p className="text-xs text-on-surface-variant">Seri Prefix</p>
                <p className="text-on-surface font-medium font-mono text-sm">{b.seriPrefix}</p>
              </div>
              <div>
                <p className="text-xs text-on-surface-variant">Üretici</p>
                <p className="text-on-surface text-sm">{b.uretici}</p>
              </div>
              <div>
                <p className="text-xs text-on-surface-variant">Üretim Tarihi</p>
                <p className="text-on-surface text-sm">{new Date(b.uretimTarihi).toLocaleDateString("tr-TR")}</p>
              </div>
            </div>
            {b.tahsisFirma && (
