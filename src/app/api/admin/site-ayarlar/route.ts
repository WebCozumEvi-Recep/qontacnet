...
export async function PUT(req: NextRequest) {
  ...
  const data = {
    ...
    saticiUnvan: String(body.saticiUnvan ?? "").trim().slice(0, 300),
    saticiAdres: String(body.saticiAdres ?? "").trim().slice(0, 500),
    saticiTelefon: String(body.saticiTelefon ?? "").trim().slice(0, 50),
    saticiEposta: String(body.saticiEposta ?? "").trim().slice(0, 200),
    saticiVergiDairesi: String(body.saticiVergiDairesi ?? "").trim().slice(0, 100),
    saticiVergiNo: String(body.saticiVergiNo ?? "").trim().slice(0, 20),
    saticiMersis: String(body.saticiMersis ?? "").trim().slice(0, 30),
    saticiIadeAdresi: String(body.saticiIadeAdresi ?? "").trim().slice(0, 500),
    saticiTeslimatMasrafi: String(body.saticiTeslimatMasrafi ?? "Satıcıya").trim().slice(0, 50) || "Satıcıya",
